...
public class IncidentsControllerTests {
	...
	public void shouldSearchIncidentsSuccessfully() throws Exception {
		...
		IncidentsDTO incident1 = new IncidentsDTO(
				UUID.randomUUID(), title, "Description 1", status, criticality, serviceId, assigneeId,
				null);
		IncidentsDTO incident2 = new IncidentsDTO(
				UUID.randomUUID(), title, "Description 2", status, criticality, serviceId, assigneeId,
				null);
	...
	public void shouldSearchIncidentsByStatusOnly() throws Exception {
		...
		IncidentsDTO incident = new IncidentsDTO(
				UUID.randomUUID(), "Test", "Desc", status, CriticalityEnum.LOW, UUID.randomUUID(), null,
				null);
	...
	public void shouldSearchIncidentsByCriticalityOnly() throws Exception {
		CriticalityEnum criticality = CriticalityEnum.CRITICAL;
		IncidentsDTO incident = new IncidentsDTO(
				UUID.randomUUID(), "Critical", "Desc", IncidentStatusEnum.OPEN, criticality,
	...
	public void shouldSearchIncidentsByServiceIdOnly() throws Exception {
		UUID serviceId = UUID.randomUUID();
		IncidentsDTO incident = new IncidentsDTO(
				UUID.randomUUID(), "Test", "Desc", IncidentStatusEnum.OPEN, CriticalityEnum.MEDIUM,
				serviceId, null,
				null);
	...
	public void shouldSearchIncidentsByTitleOnly() throws Exception {
		...
		IncidentsDTO incident = new IncidentsDTO(
				UUID.randomUUID(), title, "Desc", IncidentStatusEnum.OPEN, CriticalityEnum.CRITICAL,
				UUID.randomUUID(),
				null, null);
	...
	@Test
	@DisplayName("Should return 200 OK when status is updated successfully")
	void patchIncidentStatus_Success() throws Exception {
		UUID id = UUID.randomUUID();
		...
		when(incidentsService.updateIncidentStatus(eq(id), eq(newStatus)))
	...
	@Test
	@DisplayName("Should return error status when transition is invalid")
